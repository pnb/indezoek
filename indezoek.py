...
def make_tables(sqlite_cursor: sqlite3.Cursor):
    sqlite_cursor.execute('''CREATE TABLE IF NOT EXISTS docs (
        doc_id INTEGER PRIMARY KEY,
        path TEXT UNIQUE
    );''')
    sqlite_cursor.execute('''CREATE TABLE IF NOT EXISTS words (
...
def index_doc(sqlite_cursor: sqlite3.Cursor, path: str):
    with open(path, 'r', encoding='utf8', errors='ignore') as infile:
        txt = ''.join([l for l in infile.read().lower() if l in ALPHABET or l in SPACES])
    words = txt.split()
    word_counts = Counter(words)
    # Save to DB
    sqlite_cursor.execute('INSERT INTO docs (path) VALUES (?);', (path,))
    doc_id = sqlite_cursor.lastrowid
    sqlite_cursor.executemany('INSERT INTO words (doc_id, word, count) VALUES (?, ?, ?)',
...
def search_docs(sqlite_cursor: sqlite3.Cursor, terms: list[str], quiet: bool = False):
    ...
    for i, word in enumerate(term_words):
        ...
        sqlres = sqlite_cursor.execute('SELECT doc_id FROM words WHERE word = ?', (word,))
        if i == 0:
        ...
        for i, path in enumerate(paths):
            ...
            for term in terms:
                if term not in term_words:
                    ...
                    if firstmatch < 0 or txtlower[firstmatch - 1] in ALPHABET or \
                            txtlower[firstmatch + matchlen] in ALPHABET:
...
if __name__ == '__main__':
    ...
    ap.add_argument('--add-file-list', metavar='PATH',
                    help='Path to newline-delimited list of paths to add')
    ap.add_argument('--search', metavar='TERM', nargs='+',
    ...
    ap.add_argument('--quiet', action='store_true', help='Output only results in search')
    args = ap.parse_args()
